...
QPoint round_8(const QPoint& point) {
  ...
  // Make sure negative values don't get biased towards zero.
  return floor_8(point + QPoint(4, 4));
}
...
/**
 * @brief Floors the coordinates of a point down to the nearest lower multiple
 * of given coordinates.
 *
 * floor(QPoint(15, -2), QSize{8, 8) is QPoint(8, -8).
 *
 * @param point A point.
...
QPoint floor(const QPointF& point, const QSize& size) {
  ...
  return floor(point.toPoint(), size);
}
